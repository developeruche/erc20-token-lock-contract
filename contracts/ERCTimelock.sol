...
contract Timelock {
    address lockToken; // this is the address of the token the manager want to lock (NRF)
    address admin;
    uint256 lockTime; // this is the duration the tokens would be locked for in this contract
    bool isTokenLocked;
    ...
    /// @param _lockTime: this is the duration of time (In year) the token would be locked for in this contract
    function makeDeposit(uint256 _tokenAmount, uint256 _lockTime) public {
        if(isTokenLocked) {
